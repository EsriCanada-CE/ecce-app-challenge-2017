...
//////////////////////////////////////
	...
	calculateEstimateNEW: function (event, text) {
		var FuleEcon, TravDist, EMF;
		estimate = 0;
		var calc = this.doCalculationNEW;
		
		FuleEcon = document.getElementById("FE").innerHTML;
		TravDist = document.getElementById("TD").innerHTML;
		EMF = document.getElementById("EF").innerHTML;

		FuleEcon = this.Fule_xxx.value;
		TravDist = this.Dist_xxx.value;
		EMF = this.EF_xxx.value;
		
		document.getElementById("FE").innerHTML = FuleEcon;
		document.getElementById("TD").innerHTML = TravDist;
		document.getElementById("EF").innerHTML = EMF;


		calc(estimate, FuleEcon, TravDist, EMF);
	},
	
	doCalculationNEW: function (estimate, FuleEcon, TravDist, EMF) {
		
console.error("doCalculationNEW");
console.error(String(FuleEcon) + "x" + String(TravDist) + "=" + String(TravDist*FuleEcon));

		estimate = (TravDist/FuleEcon) * EMF
		estimate = estimate.toFixed(2);
		//area = parseFloat(area);
		//area = area.toFixed(1);
		document.getElementById("FE").innerHTML = FuleEcon;
...
// pass change event into the calculateEstimate function
	...
	onUPDATE: function () {
	  this.calculateEstimateNEW("click");
	},
...
// set onblur event to execute calculation based on a change in the area textbox
 ...
 // set onblur event to execute calculation based on a change in the area textbox
		var FE;
		FE = this.Fule_xxx;
		this.own(on(FE, "blur", lang.hitch(this, function() {this.onUPDATE();})));
 // set onblur event to execute calculation based on a change in the area textbox
		var TD;
		TD = this.Dist_xxx;
		this.own(on(TD, "blur", lang.hitch(this, function() {this.onUPDATE();})));
		
// set onblur event to execute calculation based on a change in the area textbox
		var EF;
		EF = this.EF_xxx;
		this.own(on(EF, "blur", lang.hitch(this, function() {this.onUPDATE();})));
